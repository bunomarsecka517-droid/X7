...
class ExecutionEngine {
    constructor(rpcUrls, privateKey, pimlicoApiKey) {
        this.pimlicoUrl = 'https://pimlico.io';
        this.executionLog = [];
        ...
        this.signer = privateKey ? new ethers_1.ethers.Wallet(privateKey) : null;
        this.pimlicoApiKey = pimlicoApiKey;
    }
    async executeViaFlashbots(transactions, chain = 'polygon') {
        if (!this.signer) {
    ...
    async executeViaStandardRPC(transactions, chain) {
        try {
            ...
            tx.gasLimit = ethers_1.ethers.BigNumber.from(300000);
